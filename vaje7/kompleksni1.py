...
def process_hours(hours, row):
    day_list = {"Monday":0, "Tuesday":2, "Wednesday":4, "Thursday":6, "Friday":8, "Saturday":10, "Sunday":12}
    index = day_list[row["day"]]
    val1 = process_time(row["open time"])
    val2 = process_time(row["close time"])
    return index, val1, val2


def razsiri_z_hours(business_ids):
    hours = PODATKI["hours"]
    hours_table = np.zeros((len(business_ids), 14)) + 30
    for i, id in enumerate(business_ids):
        rows = hours.loc[hours["business_id"] == id]
        for j, row in rows.iterrows():
           index, val1, val2 = process_hours(hours, row)
           hours_table[i,index] = val1
           hours_table[i,index+1] = val2
    return hours_table
